...
) 
{
  ...
  std::vector<uint8_t> packed_data(n_rows * n_cols);
  
  // We use raw pointer access for speed during packing
  const int* src_ptr = &data[0];
  uint8_t* dst_ptr = packed_data.data();
  size_t total_cells = (size_t)n_rows * n_cols;
  
  for(size_t i = 0; i < total_cells; ++i) {
    int val = src_ptr[i];
    // Safety check: Ensure values fit in uint8
...
struct BeamWorker : public Worker {
  ...
  BeamWorker(const std::vector<uint8_t>& packed_data,
             const IntegerMatrix& combos,
             const NumericVector& target,
             std::vector<float>& results)
    : data_ptr(packed_data.data()), 
      n_rows(target.size()), 
      combinations(combos),
      target(target),
      k_items(combos.ncol()), 
...
NumericVector evaluate_beam_cpp(
  ...
  // --- DISPATCHER ---
  if (na_rm) {
    BeamWorker<true> worker(packed_data, combinations, target, results);
    parallelFor(0, n_combos, worker);
  } else {
    BeamWorker<false> worker(packed_data, combinations, target, results);
    parallelFor(0, n_combos, worker);
  }
